...
<script>
  ...
  import {MENU} from './system.config'
  ...
  export default {
    ...
    methods: {
      ...
      getMenuBindRole() {  // 获取 权限关联的角色(下方表格数据)
        api.permission.getMenuBindRole(this.query.permissionId).then(data => {
        ...
        let btnHandlers = {
          ...
          btnDelete: (row) => {
            this.$confirm(`是否删除:${row.name}？`, '提示', {
              ...
              api.permission.delete(this.getIdListByRow(row)).then(() => {
                this.$message({
                  message: '删除成功!',
                  type: 'success'
                })
          ...
          btnRemove: (row) => {
            this.$confirm(`是否解除关联: ${row.role} ？`, '提示', {
              iconClass: 'fa fa-question-circle'
            }).then(() => {
              api.permission.removeRel({
