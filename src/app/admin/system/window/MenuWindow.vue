...
<script>
  ...
  export default {
    ...
    data() {
      return {
        formdata: {...MENU.FORM_DATA},
        forminfo: MENU.FORM_INFO,
        formOptions: MENU.FORM_OPTIONS,
        rules: MENU.RULES,
        windowOptions: {
          title: null,
          width: '70%'
    ...
    methods: {
      open(row = {id: null}) {
        if (row.id) {
          this.windowOptions.title = '编辑菜单'
          this.formdata = {...row}
          this.$refs.window.open()
        } else if (row.name) {
          this.$refs.window.open()
          this.$nextTick(() => {
            this.windowOptions.title = '新增菜单'
            this.formdata = {...row}
          })
        } else {
          this.$refs.window.open()
